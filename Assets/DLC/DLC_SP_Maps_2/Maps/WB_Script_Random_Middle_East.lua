------------------------------------------------------------------------------
--	FILE:	 WB_Script_Random_Middle_East.lua
--	AUTHOR:  Bob Thomas
--	PURPOSE: Hybrid script - Randomizes the Middle East.
------------------------------------------------------------------------------
...
function MultilayeredFractal:ApplyTectonics(args)
	...
	--
	local extra_mountains = args.extra_mountains or 4;
...
function TerrainGenerator:InitFractals()
	...
	self.iDesertTop = self.deserts:GetHeight(75);
	self.iDesertBottom = self.deserts:GetHeight(65);

	self.plains = Fractal.Create(	self.iWidth, self.iHeight, 
									3, self.fractalFlags, 
									self.fracXExp, self.fracYExp);
									
	self.iPlainsTop = self.plains:GetHeight(100);
	self.iPlainsBottom = self.plains:GetHeight(70);
...
function TerrainGenerator:GenerateTerrainAtPlot(iX,iY)
	...
	if desertVal >= self.iDesertTop then
		terrainVal = self.terrainGrass;
	elseif desertVal >= self.iDesertBottom or plainsVal >= self.iPlainsBottom then
