------------------------------------------------------------------------------
--	FILE:	 WB_Script_Random_Western_Europe.lua
--	AUTHOR:  Bob Thomas
--	PURPOSE: Hybrid script - Randomizes numerous things for Western Europe.
------------------------------------------------------------------------------
...
function MultilayeredFractal:ApplyTectonics(args)
	...
	for x = 0, self.iW - 1 do
		for y = 0, self.iH - 1 do
			...
			elseif self.wholeworldPlotTypes[i] == PlotTypes.PLOT_LAND then -- Add new hills and mountains only on existing flats.
				local mountainVal = self.mountainsFrac:GetHeight(x, y);
...
function TerrainGenerator:InitFractals()
	...
	self.iPlainsTop = self.plains:GetHeight(97);

	self.grass = Fractal.Create(	self.iWidth, self.iHeight, 
									2, self.fractalFlags, 
									-1, -1);

	self.iGrassTop = self.grass:GetHeight(60);
...
function TerrainGenerator:GenerateTerrainAtPlot(iX,iY)
	...
	if terrainVal ~= self.terrainTundra then
		local desVal = self.plains:GetHeight(iX, iY);
		local plainsVal = self.grass:GetHeight(iX, iY);
		if desVal >= self.iPlainsTop then
			terrainVal = self.terrainDesert;
		elseif plainsVal >= self.iGrassTop then
