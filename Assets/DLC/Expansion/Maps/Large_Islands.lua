...
function GetMapScriptInfo()
	...
	return {
		Name = "TXT_KEY_MAP_LARGE_ISLANDS",
...
function MultilayeredFractal:GeneratePlotsByRegion()
	...
	for group = 1, iNumGroups do
		if iNumCellsInUse >= iNumCellTarget or cannot_find_valid_cell == true then -- Map has reeached desired island population.
										...
										if cell_data[diceroll + 3] == false then
											if cell_data[diceroll + iNumCellColumns + 1] == false then
												if cell_data[diceroll + iNumCellColumns + 2] == false then
													if cell_data[diceroll + iNumCellColumns + 3] == false then -- All cells are open.
											...
											if cell_data[diceroll + (iNumCellColumns * 2)] == false then
												if cell_data[diceroll + (iNumCellColumns * 2) + 1] == false then
													if cell_data[diceroll + (iNumCellColumns * 3)] == false then
														if cell_data[diceroll + (iNumCellColumns * 3) + 1] == false then -- All cells are open.
															anchor_cell = diceroll;
															found_unoccupied_cell = true;
														end
													end
									...
									if cell_data[diceroll + 1] == false and cell_data[diceroll + 2] == false then
										if cell_data[diceroll + iNumCellColumns + 1] == false then
											if cell_data[diceroll + (iNumCellColumns * 2)] == false then
												if cell_data[diceroll + (iNumCellColumns * 2) + 1] == false then
													if cell_data[diceroll + iNumCellColumns + 2] == false then
														if cell_data[diceroll + (iNumCellColumns * 2) + 2] == false then -- All cells are open.
															anchor_cell = diceroll;
															found_unoccupied_cell = true;
														end
													end
		...
		if cannot_find_valid_cell == false then
			...
			elseif formation_type == 3 then
				...
				cell_data[anchor_cell + iNumCellColumns], cell_data[anchor_cell + iNumCellColumns + 1], cell_data[anchor_cell + iNumCellColumns + 2] = true, true, true;
				cell_data[anchor_cell + (iNumCellColumns * 2)], cell_data[anchor_cell + (iNumCellColumns * 2) + 1], cell_data[anchor_cell + (iNumCellColumns * 2) + 2] = true, true, true;
...
function AssignStartingPlots:BalanceAndAssign()
	-- Customized for Large Islands. All start biases disabled.
