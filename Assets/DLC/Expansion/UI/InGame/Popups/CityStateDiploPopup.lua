...
local kiOriginalPopupY = Controls.Popup:GetSizeY();
local kiOriginalButtonScrollPanelY = Controls.ButtonScrollPanel:GetSizeY();
local kiNoAction = 0;
local kiMadePeace = 1;
...
function UpdateButtonStack()
	local tStacks = { Controls.GiveStack, Controls.TakeStack, Controls.ButtonStack };
    
	-- Update stack sizes
	for i,c in pairs(tStacks) do
		c:CalculateSize();
		c:ReprocessAnchoring();
	end
	Controls.ButtonScrollPanel:CalculateInternalSize();
	
	-- Reset the size of the popup to reflect how big the visible button stack is
	local iVisibleSizeY = 0;
	for i,c in pairs(tStacks) do
		if (not c:IsHidden()) then
			local iY = c:GetSizeY();
			if (iY > iVisibleSizeY) then
				iVisibleSizeY = iY;
			end
		end
	end
	Controls.Popup:SetSizeY(kiOriginalPopupY - kiOriginalButtonScrollPanelY + iVisibleSizeY);
