------------------------------------------------------------------------------
--	FILE:	 WB_Script_Canada.lua
--	AUTHOR:  Bob Thomas
--	PURPOSE: Hybrid script - Customized to randomize Canada.
------------------------------------------------------------------------------
...
function MultilayeredFractal:GeneratePlotsByRegion()
	...
	local args = {
		world_age = world_age,
		adjust_plates = 1.5,			-- Increases both hills and mountains.
...
function TerrainGenerator:InitFractals()
	...
	self.iPlainsTop = self.plains:GetHeight(55);

	self.grass = Fractal.Create(	self.iWidth, self.iHeight, 
									2, self.fractalFlags, 
									-1, -1);

	self.iGrassTop = self.grass:GetHeight(90);
...
function TerrainGenerator:GenerateTerrainAtPlot(iX,iY)
	...
	if terrainVal == self.terrainGrass then
		...
		elseif desertVal >= self.iGrassTop then
...
function FeatureGenerator:AdjustTerrainTypes()
	...
	for y = math.floor(self.iGridH / 3), height do
		for x = 0, width do
			local plot = Map.GetPlot(x, y);
